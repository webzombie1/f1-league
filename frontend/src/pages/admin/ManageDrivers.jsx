  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={create} className="bg-[#1E2642] border border-[#2A3458] rounded-xl p-5 space-y-3">
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-xs text-[#8892A8] uppercase tracking-wider mb-1">EA Tag</label>
            <input value={eaTag} onChange={e => setEaTag(e.target.value)} placeholder="EA gamertag" className={inputCls} />
          </div>
          <div className="w-36">
            <label className="block text-xs text-[#8892A8] uppercase tracking-wider mb-1">Platform</label>
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-xs text-[#8892A8] uppercase tracking-wider mb-1">Discord Name</label>
            <input value={discordName} onChange={e => setDiscordName(e.target.value)} placeholder="username#1234" className={inputCls} />
          </div>
          <div className="flex-1">
            <label className="block text-xs text-[#8892A8] uppercase tracking-wider mb-1">Discord Profile URL</label>
      ...
      <div className="space-y-2">
        {drivers.map(d => (
          <div key={d.id} className="bg-[#1E2642] border border-[#2A3458] rounded-xl overflow-hidden">
            {/* Summary row — click to expand */}
            <div
              className="p-4 flex items-center justify-between cursor-pointer hover:bg-[#253052] transition-colors"
            ...
            >
              <div className="flex items-center gap-3">
                {d.photo_url ? (
                  ...
                  <div className="w-8 h-8 rounded-full bg-[#2A3458] shrink-0 flex items-center justify-center">
                    <svg className="w-4 h-4 text-[#555F78]" fill="currentColor" viewBox="0 0 24 24">
                ...
                {d.discord_name && <span className="text-[#555F78] text-xs ml-2 font-mono">({d.discord_name})</span>}
              </div>
              <svg className={`w-4 h-4 text-[#555F78] transition-transform ${editingId === d.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            {/* Expanded edit panel */}
            {editingId === d.id && (
              <div className="border-t border-[#2A3458] p-4 space-y-3 bg-[#1A2240]">
                {/* Photos */}
                <div className="flex gap-4 items-start mb-4">
                  <div className="text-center">
                    <p className="text-[8px] text-[#8892A8] uppercase tracking-wider mb-1">Thumbnail</p>
                    <div className="w-16 h-16 rounded-lg bg-[#2A3458] overflow-hidden mb-1">
                  ...
                  <div className="text-center">
                    <p className="text-[8px] text-[#8892A8] uppercase tracking-wider mb-1">Standing</p>
                    <div className="w-16 h-20 rounded-lg bg-[#2A3458] overflow-hidden mb-1">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <label className="block text-xs text-[#8892A8] uppercase tracking-wider mb-1">Discord URL</label>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button onClick={() => setEditingId(null)} className="text-sm text-[#8892A8] hover:text-[#E8ECF4]">Cancel</button>
