...
async def generate_celebration_hero(race_id: int, request: Request):
    ...
    prompt = (
        ...
        "COMPOSITION RULES:\n"
        "- The driver must sit roughly in the RIGHT HALF of the frame, full body or "
        "  upper body, with their face entirely visible and roughly on the upper-third "
        "  horizontal line.\n"
        ...
        "- The LEFT THIRD of the frame should be the SAME continuous scene (e.g. crowd, "
        "  sky, grandstand) but quieter and lower-contrast (atmospheric perspective, "
        "  motion blur, soft bokeh) so the text panel can overlay it. It must visually "
        "  belong to the same scene as the right two-thirds — not a separate photo.\n"
