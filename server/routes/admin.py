...
async def generate_celebration_hero(race_id: int, request: Request):
    ...
    prompt = (
        ...
        "  so the TOP ~25% and the BOTTOM ~25% of your image WILL BE CROPPED OUT on "
        "  most screens. Treat the central 50% horizontal band (y=25%–75%) as the "
        "  ONLY safe area. Everything that matters — face, hands, champagne bottle, "
        "  trophy, key venue signage, crowd faces — must sit fully within this band.\n"
        "- The driver's head and face must be in the upper half of the safe band "
        "  (around y=30%–45% of the full frame), NOT near the top edge. Their full "
        "  upper body and any object they're holding (bottle, trophy) must fit "
        "  within the safe band as well.\n"
        "- Iconic venue features (e.g. signage, distinctive architecture) should be "
        "  placed in the safe band, not floating near the top edge where they'll be "
        "  clipped.\n"
